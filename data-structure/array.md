...
可能遇见有些题目会出现一些矩阵的字眼，可能就需要使用到二维数据。

还有需要注意的点是，有时我们使用数组的fill方法来填充数据，但不适合于生成二维数组。

``` javascript
const arr = new Array(3).fill([1,2,3]);
arr[0][1] = 4;
console.log(arr); // [ [ 1, 4, 3 ], [ 1, 4, 3 ], [ 1, 4, 3 ] ]
```

可以看到，使用fill来填充引用类型数据时，改的是同一个地址的数据，还是乖乖遍历把。